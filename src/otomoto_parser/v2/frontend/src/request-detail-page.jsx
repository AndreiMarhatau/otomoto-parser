...
export function RequestDetailPage() {
  ...
  return (
    <Shell title={`Request ${item.id}`} subtitle="Parser status, stored outputs, and rerun actions live here.">
      <Breadcrumbs items={[{ label: "Requests", to: "/" }, { label: "Details" }]} />
      <section className="panel">
        <div className="detail-head">
          <div className="detail-source">
            <p className="section-kicker">Source URL</p>
            <a href={item.sourceUrl} target="_blank" rel="noreferrer" className="link-break">{item.sourceUrl}</a>
          </div>
        ...
        <div className="metric-grid"><Metric label="Pages completed" value={item.pagesCompleted} /><Metric label="Listings written" value={item.resultsWritten} /><Metric label="Results" value={item.resultsReady ? "Ready" : "Not ready"} /><Metric label="Excel export" value={item.excelReady ? "Ready" : "Pending"} /></div>
        <div className="detail-actions">
          ...
          <a className={`button-link button-link-secondary ${!item.excelReady ? "button-disabled" : ""}`} href={item.excelReady ? `/api/requests/${item.id}/excel` : undefined}>Download Excel</a>
        </div>
